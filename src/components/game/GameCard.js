...
import { View, Text, TouchableOpacity, TouchableNativeFeedback, TouchableWithoutFeedback } from 'react-native'
...
const GameCard = ({ game, isTournament }) => {
    ...
    return (
        ...
        >
            <View style={tw`flex flex-col  border-[${theme.colors.border}]  border-t-[1px] py-3`}>
                {
                    isTournament &&
                ...
                <View style={tw`flex flex-row gap-1 w-full `}>
                    ...
                    <View style={tw`w-[35%] flex flex-col justify-center gap-1 items-center`}>
                        ...
                        <Text style={tw.style(`text-[${get_status_color(game.fullStatus.type.state)}]`)}>
                        ...
                        {
                            "video" in game &&
                            <Button labelStyle={{ fontSize: 14 }} mode='outlined' buttonColor={theme.colors.background} textColor={theme.colors.text} onPress={() => { }}>Videos</Button>
                        }
